...
namespace Neira.Bot.Modules
{
	...
	public class ModerationModule : BaseModule
	{
		...
		{
			_logger = service.GetRequiredService<ILogger<ModerationModule>>();
		...
		public async Task GetGuildConfig()
		{
			...
			var embed = new EmbedBuilder()
				...
				.WithAuthor($"Мои настройки на этом сервере.", _discord.CurrentUser.GetAvatarUrl() ?? _discord.CurrentUser.GetDefaultAvatarUrl())
		...
		public async Task ServerInfoAsync()
		{
			...
			catch (Exception ex)
			{
				...
				_logger.LogWarning(ex, "ADOnline command");
			}
		}
	}
}